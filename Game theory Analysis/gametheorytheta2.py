...
def launch_attack_channel(G,gamma,per_tx_val,path_length,source,num_corrupted,laid_back,corruption,profit,max_limit):
    ...
    for node in source:
        ...
        #theta2=0.4
        a=float((1-0.5*float(corruption/100))/3)
        print(a)
        if 0.81>float(corruption/100):
...
def main():
   ...
   # set_graph_color(G)
    ...
    num_corrupted=int(20*G.number_of_nodes()/100)
    
    num_laidback=int(corruption*G.number_of_nodes()/100)
