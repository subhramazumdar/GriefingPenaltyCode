...
def mount_griefing(G, attacker_node, node_potential_victim, set_source, set_sink_remain, budget=30000):
    """
    Exhaust several channels of the victim within the given budget
    """
    while budget > 0:
        node_select = "-1"
        min_wt = 300000000000000000000
        for neighbor in set_source :            
            # select a neighbour of victim node from the source set whose channel has to be exhausted
            if int(G.edges[node_potential_victim,neighbor]['capacity']) > 0 and min_wt > int(G.edges[node_potential_victim,neighbor]['capacity']):
                min_wt = int(G.edges[node_potential_victim,neighbor]['capacity'])
        ...
        for neighbor in set_sink_remain:
            if int(G.edges[node_potential_victim,neighbor]['capacity']) >= min_wt:
                target_select = neighbor
...
def attack_best_case_earning(G,attacker_node,node_potential_victim,budget=30000):    
    ...
    """
    mount griefing attack by exhausting several channels of the chosen victim nodes 
    till the budget becomes 0 or till no condition mentioned in the function holds true
    """
