...
def launch_attack_channel(G,gamma,per_tx_val,path_length,max_limit):
    ...
    while time<max_limit and path_now<path_length:
            ...
            for node in G.neighbors(current):
                ...
                if G.edges[node,current]['capacity']>=per_tx_val and time+G.nodes[node]['time']<=max_limit:
                    current=node
    ...
    return cum_penalty+per_tx_val,path_now
...
def main():
   ...
   # set_graph_color(G)
    ...
    budget,path_return=launch_attack_channel(G_tmp,gamma,per_tx_val,path_length,2016)
    
    f1.write(sys.argv[1]+" "+str(per_tx_val)+" "+str(gamma)+" "+str(path_return)+" "+str(budget)+"\n")
    G_tmp=G.copy()
