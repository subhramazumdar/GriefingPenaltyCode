...
def launch_attack_channel(G,gamma,per_tx_val,path_length,adv_budget,max_limit):
    ...
    while adv_budget>0.01:
        ...
        while time<max_limit and path_now<path_length:
            ...
            for node in G.neighbors(current):
                ...
                if time+G.nodes[prev_current]['time']+G.nodes[node]['time']<=max_limit:
                    ...
                    if G.edges[prev_current,node]['deposit2']>10:
        ...
        collateral=collateral-final_value
        adv_budget=adv_budget-final_value-flow
...
def main():
   ...
   # set_graph_color(G)
    ...
    f1.write(sys.argv[1]+" "+str(per_tx_val)+" "+str(adv_budget)+" "+str(budget)+" "+str(budget1)+"\n")
    G_tmp=G.copy()
